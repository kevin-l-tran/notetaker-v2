...
const targets = [
	{ targetId: "t1", phrases: ["t1-label1", "t1-label2", "t1-label3"] },
	{ targetId: "t2", phrases: ["t2-label1", "t2-label2", "t2-label3"] },
	{ targetId: "t3", phrases: ["t3-label1", "t3-label2", "t3-label3"] },
];
...
describe("getAutolinkReplacements", () => {
	...
	it("ignores automatic local links targeting IDs not included in the targets", () => {
		const text = "[[~t1-label1|t4]]";
		const replacements = getAutolinkReplacements(text, targets);
	...
	it("only returns edits for automatic links when link targets are mixed", () => {
		const text = "[[~t1-label1|t1]][[~nonlabel|nontarget]][[~t2-label1|t2]]";
		const replacements = getAutolinkReplacements(text, targets);

		expect(replacements).toStrictEqual([
			{ start: 0, end: 17, replacement: "t1-label1" },
			{ start: 40, end: 57, replacement: "t2-label1" },
		]);
	});

	it("returns no edits when the target list is empty", () => {
		const text = "[[~t1-label1|t1]][[~t2-label1|t2]][[~t3-label1|t3]]";
